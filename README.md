...
| `/save` | 保存当前会话 |
| `/mode [name]` | 设置运行模式：`build` / `plan` / `accept_edits` |
...
| `--mode` | 运行模式 (default/build/plan/accept_edits) |
| `--build` | 快捷启用 Build 模式 |
| `--plan` | 快捷启用规划模式 |
| `--resume, -r` | 恢复上次会话 |
| `--session` | 指定会话 ID |
