...
- **複数の実行モード**: デフォルト、YOLO、計画モード、自動編集承認モード
...
# YOLOモード（自動実行）
foxcode --yolo "Flaskアプリケーションを作成してください"
...
| `/save` | 現在のセッションを保存 |
| `/mode [name]` | 実行モードを設定：`yolo` / `plan` / `accept_edits` |
...
| `--mode` | 実行モード (default/yolo/plan/accept_edits) |
| `--yolo` | YOLOモードをクイック有効化 |
| `--plan` | 計画モードをクイック有効化 |
