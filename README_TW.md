...
| `/save` | 儲存當前會話 |
| `/mode [name]` | 設定執行模式：`yolo` / `plan` / `accept_edits` |
...
| `--mode` | 執行模式 (default/yolo/plan/accept_edits) |
| `--yolo` | 快捷啟用 YOLO 模式 |
| `--plan` | 快捷啟用規劃模式 |
| `--resume, -r` | 恢復上次會話 |
| `--session` | 指定會話 ID |
