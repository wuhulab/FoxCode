...
def _init_watchdog() -> ProcessWatchdog:
    ...
    def on_memory_warning(memory_mb: float) -> None:
        """内存超限回调"""
        logger.warning(f"⚠️ 内存使用过高 ({memory_mb:.1f}MB)，建议保存会话并重启")
        console.print(f"\n[yellow]⚠️ 内存使用较高: {memory_mb:.1f}MB[/yellow]")
        if memory_mb > 800:  # 超过800MB时强烈警告
            console.print("[red]内存使用非常高，可能存在内存泄漏！[/red]")
...
def _handle_unexpected_error(error: Exception) -> None:
    ...
    try:
        with open(crash_report_path, "a", encoding="utf-8") as f:
            f.write(f"\n{'=' * 80}\n")
            f.write(
                f"时间: {logging.Formatter().formatTime(logging.LogRecord('', '', '', '', '', '', ''))}\n"
            )
...
使用 diff 算法保护项目原有注释，默认开启。AI 编辑文件后会自动恢复
项目原有的注释（包括行注释、块注释、文档字符串），保护代码可维护性。
...
- `/cp reset` - 重置统计数据
- `/cp config` - 查看配置
- `/cp config key=value` - 修改配置（如 `/cp config fuzzy_threshold=0.7`）

支持的扩展名：`.py .js .ts .jsx .tsx .java .go .rs .c .cpp .h .hpp .cs .rb
.php .swift .kt .scala .lua .r .sql .sh .bash .html .xml .css .scss .yaml
.yml .toml .ini .json 等 30+ 种文件类型`。
...
@click.option(
    ...
    "use_tui",
    is_flag=True,
    help="启动轻量级 TUI 终端界面（基于 Textual，类 opencode/uv 风格）",
)
...
def tui(
    ...
    """启动轻量级 TUI 终端界面（类 opencode/uv 风格）。

    基于 Textual 的现代化终端 UI，无需浏览器，内存占用极低。
...
async def _run_single_prompt(
    agent: FoxCodeAgent, prompt: str, config: Config | None = None
) -> None:
    """
    运行单次提示
        ...
        try:
            ...
            try:
                ...
                if _watchdog and request_id:
                    _watchdog.record_request_success(request_id, response_time_ms)
                ...
                response_time_ms = (time.time() - request_start_time) * 1000
                if _watchdog and request_id:
                    _watchdog.record_request_failure(request_id, chat_error, response_time_ms)
            ...
            if consecutive_errors >= max_consecutive_errors:
                logger.critical(
                    f"连续错误次数达到上限 ({max_consecutive_errors})，可能存在系统性问题"
                )
                if config.output_topic == OutputTopic.MINIMALISM:
...
def _handle_command(command: str, agent: FoxCodeAgent, config: Config) -> bool:
    ...
    elif cmd_name == "/cp" or cmd_name == "/comment-protect" or cmd_name == "/protect-comments":
        _handle_comment_protect_command(config, cmd_arg)
...
def _handle_design_command(cmd_arg: str | None) -> None:
    ...
    if not cmd_arg:
        status = (
            "[green]已启用[/green]" if design_mode_manager.is_enabled() else "[dim]未启用[/dim]"
        )
        console.print("[cyan]DESIGN.md 设计规范工具[/cyan]")
        console.print(f"设计规范遵守模式: {status}")
        console.print(
            "[dim]用法: /design on | /design off | /design status | /design lint <file> | /design spec | /design export <file> <format>[/dim]"
        )
        return
    ...
    if sub_cmd == "lint":
        ...
        console.print(
            f"[bold]验证结果:[/bold] {report.summary['errors']} 错误, {report.summary['warnings']} 警告, {report.summary['infos']} 信息"
        )
...
# 处理 /cp（注释保护）命令
def _handle_comment_protect_command(config: Config, cmd_arg: str | None) -> None:
    """
    处理 /cp 命令 - 注释保护功能

    使用 diff 算法保护项目原有注释，默认开启。AI 编辑文件后会自动
    恢复原始注释，保护代码可维护性。
    ...
    if not cmd_arg:
        # 显示状态
        status = "[green]已启用[/green]" if manager.is_enabled() else "[dim]已禁用[/dim]"
        console.print("[cyan]注释保护 (Comment Protection)[/cyan]")
        console.print(f"状态: {status}")
        stats = manager.get_stats()
        console.print(f"已保护文件: {stats.files_protected}")
        console.print(f"恢复原始注释: {stats.comments_restored}")
        console.print(f"保留注释: {stats.comments_kept}")
        console.print(f"丢失注释: {stats.comments_lost}")
        console.print(f"AI 新增注释: {stats.new_comments_added}")
        if stats.last_protected_file:
            console.print(f"[dim]最近文件: {stats.last_protected_file}[/dim]")
        console.print()
        console.print(
            "[dim]用法: /cp [on | off | status | stats | reset | config [key=value]][/dim]"
        )
        return
    ...
    if sub_cmd in ("on", "enable", "true", "1"):
        result = manager.enable()
        # 同步到 config
        if hasattr(config, "protect_comments"):
            config.protect_comments.enabled = True
        console.print(f"[green]{result}[/green]")

    elif sub_cmd in ("off", "disable", "false", "0"):
        result = manager.disable()
        if hasattr(config, "protect_comments"):
            config.protect_comments.enabled = False
        console.print(f"[yellow]{result}[/yellow]")

    elif sub_cmd == "status":
        if manager.is_enabled():
            console.print("[green]注释保护: 已启用[/green]")
            stats = manager.get_stats()
            console.print(f"已保护文件: {stats.files_protected}")
            console.print(f"恢复注释: {stats.comments_restored}")
            console.print(f"丢失注释: {stats.comments_lost}")
        else:
            console.print("[dim]注释保护: 已禁用[/dim]")
            console.print("[dim]使用 /cp on 启用[/dim]")

    elif sub_cmd == "stats":
        stats = manager.get_stats()
        console.print("[bold cyan]注释保护统计[/bold cyan]")
        console.print(f"  已保护文件数: {stats.files_protected}")
        console.print(f"  恢复原始注释: {stats.comments_restored}")
        console.print(f"  保留注释: {stats.comments_kept}")
        console.print(f"  丢失注释: {stats.comments_lost}")
        console.print(f"  AI 新增注释: {stats.new_comments_added}")
        console.print(f"  跳过的文件: {stats.files_skipped}")
        if stats.last_protected_file:
            console.print(f"  最近文件: {stats.last_protected_file}")
        if stats.last_result:
            console.print(f"  最近结果: {stats.last_result.summary()}")

    elif sub_cmd == "reset":
        manager.reset_stats()
        console.print("[green]注释保护统计已重置[/green]")

    elif sub_cmd == "config":
        if not sub_arg:
            # 显示当前配置
            if hasattr(config, "protect_comments"):
                pc = config.protect_comments
                console.print("[bold cyan]注释保护配置[/bold cyan]")
                console.print(f"  enabled: {pc.enabled}")
                console.print(f"  fuzzy_threshold: {pc.fuzzy_threshold}")
                console.print(f"  search_radius: {pc.search_radius}")
                console.print(f"  protect_docstrings: {pc.protect_docstrings}")
                console.print(f"  protect_inline: {pc.protect_inline}")
                console.print(f"  show_notifications: {pc.show_notifications}")
                console.print()
                console.print("[dim]用法: /cp config key=value[/dim]")
                console.print(
                    "[dim]可配置: enabled, fuzzy_threshold, search_radius, protect_docstrings, protect_inline, show_notifications[/dim]"
                )
        else:
            # 解析 key=value
            kv_parts = sub_arg.split("=", 1)
            if len(kv_parts) != 2:
                console.print("[yellow]用法: /cp config key=value[/yellow]")
                return
            key, value = kv_parts[0].strip(), kv_parts[1].strip()
            if not hasattr(config.protect_comments, key):
                console.print(f"[red]未知配置项: {key}[/red]")
                return
            # 类型转换
            current = getattr(config.protect_comments, key)
            if isinstance(current, bool):
                ...
                new_value = value
            setattr(config.protect_comments, key, new_value)
            console.print(f"[green]已设置 {key} = {new_value}[/green]")

    else:
        console.print(f"[yellow]未知子命令: {sub_cmd}[/yellow]")
        console.print("[dim]可用: on, off, status, stats, reset, config[/dim]")
...
def _handle_work_command(agent: FoxCodeAgent, config: Config, cmd_arg: str | None) -> None:
    ...
    try:
        ...
        # 查看状态
            ...
            if status["active_tasks"]:
                ...
                for task_id in status["active_tasks"]:
                    ...
                    if task:
                        console.print(
                            f"  [LOOP] {task_id}: {task.description[:40]}... ({task.current_phase})"
                        )
        ...
        # 停止任务
            ...
            console.print(
                "[yellow]用法: /work [off] | <任务描述> | status | list | records | stop <id>[/yellow]"
            )
...
def _handle_stats_command() -> None:
    ...
    try:
        ...
        table.add_row("总请求数", str(metrics.total_requests), "所有AI请求的总数")
        table.add_row(
            "成功率",
            f"{(metrics.successful_requests / max(1, metrics.total_requests) * 100):.1f}%",
            "成功完成的请求比例",
        )
        table.add_row("平均响应时间", f"{metrics.avg_response_time_ms:.0f}ms", "AI响应的平均耗时")
        table.add_row("最快响应", f"{metrics.min_response_time_ms:.0f}ms", "最快的一次响应")
        table.add_row("最慢响应", f"{metrics.max_response_time_ms:.0f}ms", "最慢的一次响应")
        table.add_row("内存使用", f"{metrics.memory_usage_mb:.1f}MB", "当前进程内存占用")
        table.add_row("CPU使用率", f"{metrics.cpu_percent:.1f}%", "当前进程CPU使用")
        table.add_row("连续错误", str(metrics.consecutive_errors), "连续失败的请求次数")
...
def _start_tui_mode(
    ...
    """启动 FoxCode 轻量级 TUI 终端界面。

    基于 Textual 的现代化终端 UI，类 opencode/uv 风格：
    - 顶部状态栏（模型 / 模式 / Token）
    - 左侧会话列表
    - 中部对话区（Markdown 渲染 + 流式输出）
    - 底部多行输入
    - 极低内存占用（无浏览器、无 Monaco）
    ...
    try:
        ...
        console.print(
            Panel(
                "[red]缺少 TUI 依赖[/red]\n\n"
                "请运行以下命令安装：\n\n"
                "  [bold]pip install 'textual>=0.47.0'[/bold]\n\n"
                "或升级 FoxCode 以获得完整可选依赖：\n\n"
                "  [bold]pip install --upgrade foxcode[/bold]",
                title="依赖缺失",
                border_style="red",
            )
        )
    ...
    if yolo:
        ...
        overrides["run_mode"] = RunMode(mode)
    if debug:
        overrides["debug"] = True

    app_config = Config.create(**overrides)
    ...
    try:
        ...
        console.print(
            Panel(
                f"[red]TUI 启动失败[/red]\n\n"
                f"错误信息: {markup.escape(str(exc))}\n\n"
                f"详细日志: {Path.home() / '.foxcode' / 'foxcode.log'}",
                title="TUI 错误",
                border_style="red",
            )
        )
        logger.error("TUI 启动失败", exc_info=True)
        sys.exit(1)
...
def _handle_index_command(agent: FoxCodeAgent, config: Config, cmd_arg: str | None) -> None:
    ...
    try:
        ...
        if cmd_arg == "status":
            ...
            files_updated = run_async(
                index.index_directory(Path(config.working_dir), incremental=True)
            )
...
def _handle_refactor_command(agent: FoxCodeAgent, config: Config, cmd_arg: str | None) -> None:
    ...
    try:
        ...
        console.print(
            Panel(
                "\n".join(
                    f"[bold]{smell.type}:[/bold] {smell.description}\n"
                    f"  位置: {smell.location}\n"
        ...
        if sub_cmd == "commit":
            ...
            if not conflicts:
                ...
                console.print(
                    Panel(
                        "\n".join(
                            f"[bold]{c.file_path}:[/bold] {c.description}" for c in conflicts
                        ),
                        title="⚠️ 冲突分析",
                        style="yellow",
                    )
    ...
    try:
        ...
        if sub_cmd == "add":
            if sub_arg1 and sub_arg2:
                ...
                console.print("\n[dim]示例:[/dim]")
                console.print(
                    '  /space add "Windows路径问题" "在Windows上使用反斜杠路径，但Python推荐使用正斜杠或Path对象"'
                )
            return
    ...
    try:
        ...
        if first_arg == "list":
            ...
            if is_minimalism:
                ...
                for skill in skills:
                    table.add_row(
                        "Skill", skill.name, skill.description[:50] if skill.description else ""
                    )
        ...
        if first_arg == "skills":
            ...
            if is_minimalism:
                ...
                for skill in skills:
                    table.add_row(
                        skill.name,
                        skill.description[:50] if skill.description else "",
                        str(skill.path),
                    )
        ...
        # 检查是否是 GitHub URL
        if first_arg.startswith("https://github.com/") or first_arg.startswith(
            "http://github.com/"
        ):
            url = first_arg
                ...
                if is_minimalism:
                    ...
                    if result.skills:
                        ...
                        for skill in result.skills:
                            console.print(
                                f"  - [cyan]{skill.name}[/cyan]: {skill.description[:50]}"
                            )
    ...
    try:
        ...
        if result.status == UpdateStatus.UP_TO_DATE:
            ...
            if result.message and "需要退出后更新" in result.message:
                # 从 message 中提取脚本路径
                script_path = (
                    result.message.split("更新脚本: ")[-1] if "更新脚本: " in result.message else ""
                )
